...
const reminderOptions = [{ value: '15min', label: '15 minutos antes' }, { value: '30min', label: '30 minutos antes' }, { value: '1h', label: '1 hora antes' }, { value: '1day', label: '1 día antes' }];
...
const Bell = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9" /><path d="M13.7 21a2 2 0 0 1-3.4 0" /></svg>;
  ...
  return <>
    ...
    {showDayEvents && <div onClick={() => setShowDayEvents(false)} style={{ position: 'fixed', inset: 0, zIndex: 70, display: 'flex', alignItems: 'flex-end', background: 'rgba(0,0,0,0.32)' }}>
      <section onClick={event => event.stopPropagation()} style={{ width: '100%', maxHeight: 'calc(100dvh - 112px)', overflowY: 'auto', padding: '20px 16px calc(20px + 86px)', borderRadius: '24px 24px 0 0', background: '#FFFDF6', boxShadow: '0 -4px 18px rgba(0,0,0,0.14)', boxSizing: 'border-box' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', marginBottom: '14px' }}><div><h2 style={{ margin: 0, color: '#1F2937', fontSize: '19px' }}>Eventos del día</h2><p style={{ margin: '3px 0 0', color: '#6B7280', fontSize: '13px' }}>{new Date(`${date}T12:00:00`).toLocaleDateString('es-AR', { day: 'numeric', month: 'long' })}</p></div><button onClick={openNewEvent} style={{ border: 'none', borderRadius: '999px', padding: '9px 12px', color: '#FFFFFF', background: '#D946EF', fontWeight: 700, fontSize: '12px', cursor: 'pointer' }}>+ Agregar</button></div>
        <div style={{ display: 'grid', gap: '8px' }}>{dayEvents.map(event => {
          ...
          return <article key={event.id} style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) auto', columnGap: '10px', padding: '11px 12px', border: '1px solid #E5E7EB', borderLeft: `4px solid ${event.type === 'sintoma' ? '#EAB308' : '#22C55E'}`, borderRadius: '12px', background: '#FFFFFF', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
            <button onClick={() => openEvent(event)} style={{ gridColumn: '1 / -1', padding: 0, margin: '0 0 7px', border: 'none', background: 'transparent', textAlign: 'left', color: '#1F2937', fontSize: '16px', lineHeight: 1.15, fontWeight: 700, cursor: 'pointer' }}>{event.name || 'Evento'}</button>
            <div style={{ display: 'grid', gap: '5px', minWidth: 0 }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '5px', color: '#6B7280', fontSize: '11px', fontWeight: 700 }}>Fecha<input aria-label={`Fecha de ${event.name}`} type="date" value={localDateValue(event.date)} onChange={change => updateEvent(event.id, { date: new Date(`${change.target.value}T12:00:00`).toISOString() })} style={{ minWidth: 0, border: 'none', padding: 0, background: 'transparent', color: '#374151', fontFamily: 'inherit', fontSize: '11px' }} /><span style={{ color: '#D946EF', display: 'flex' }}><Pencil /></span></label>
              <label style={{ display: 'flex', alignItems: 'center', gap: '5px', color: '#6B7280', fontSize: '11px', fontWeight: 700 }}>Hora<input aria-label={`Hora de ${event.name}`} type="time" value={event.time || '09:00'} onChange={change => updateEvent(event.id, { time: change.target.value })} style={{ minWidth: 0, border: 'none', padding: 0, background: 'transparent', color: '#374151', fontFamily: 'inherit', fontSize: '11px' }} /><span style={{ color: '#D946EF', display: 'flex' }}><Pencil /></span></label>
            </div>
            {event.type !== 'sintoma' && <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '4px', color: '#D946EF', maxWidth: '142px' }}><Bell /><div style={{ display: 'grid', gap: '4px' }}>{notifications.map((value, index) => <div key={`${value}-${index}`} style={{ display: 'flex', alignItems: 'center', gap: '3px' }}><select aria-label={`Recordatorio ${index + 1} de ${event.name}`} value={value} onChange={change => updateNotifications(event, notifications.map((notification, notificationIndex) => notificationIndex === index ? change.target.value : notification))} style={{ width: '108px', padding: '4px 3px', border: '1px solid #D1D5DB', borderRadius: '6px', background: '#FFFFFF', color: '#374151', fontSize: '10px' }}>{reminderOptions.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select><button aria-label="Quitar recordatorio" onClick={() => updateNotifications(event, notifications.filter((_, notificationIndex) => notificationIndex !== index))} style={{ border: 'none', padding: 0, background: 'transparent', color: '#D946EF', fontSize: '18px', lineHeight: 1, cursor: 'pointer' }}>×</button></div>)}</div><button aria-label="Agregar recordatorio" onClick={() => updateNotifications(event, [...notifications, '1h'])} style={{ border: 'none', padding: '0 2px', background: 'transparent', color: '#D946EF', fontSize: '19px', lineHeight: 1, cursor: 'pointer' }}>+</button></div>}
          </article>;
        })}</div>
      </section>
    </div>}
