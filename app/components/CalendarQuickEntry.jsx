    ...
    {showDayEvents && <div onClick={() => setShowDayEvents(false)} style={{ position: 'fixed', inset: 0, zIndex: 70, display: 'flex', alignItems: 'flex-end', background: 'rgba(0,0,0,0.32)' }}>
      <section onClick={event => event.stopPropagation()} style={{ width: '100%', maxHeight: 'calc(100dvh - 112px)', overflowY: 'auto', padding: '20px 16px calc(20px + 86px)', borderRadius: '24px 24px 0 0', background: '#FFFDF6', boxShadow: '0 -4px 18px rgba(0,0,0,0.14)', boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'grid', gap: '8px' }}>{dayEvents.map(event => {
          ...
          return <article key={event.id} style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) auto', columnGap: '10px', padding: '11px 12px', border: `1px solid ${event.type === 'sintoma' ? '#EAB308' : '#22C55E'}`, borderRadius: '12px', background: '#FFFFFF' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', minWidth: 0, color: '#6B7280' }}>
              <span aria-hidden="true" style={{ display: 'flex' }}><Clock /></span>
              {editingTime?.id === String(event.id) ? <input aria-label={`Hora de ${event.name}`} type="time" autoFocus value={editingTime.value} onChange={change => setEditingTime({ id: String(event.id), value: change.target.value })} onBlur={() => { const nextTime = editingTime?.value; setEditingTime(null); if (nextTime && nextTime !== (event.time || '09:00')) updateEvent(event.id, { time: nextTime }); }} onKeyDown={key => { if (key.key === 'Enter') key.currentTarget.blur(); if (key.key === 'Escape') setEditingTime(null); }} style={{ width: '82px', border: 'none', padding: 0, background: 'transparent', color: '#374151', fontFamily: 'inherit', fontSize: '13px', fontWeight: 600 }} /> : <><span style={{ color: '#374151', fontSize: '13px', fontWeight: 600 }}>{event.time || '09:00'}</span><button aria-label={`Editar hora de ${event.name}`} onClick={() => setEditingTime({ id: String(event.id), value: event.time || '09:00' })} style={{ display: 'flex', padding: 0, border: 'none', background: 'transparent', color: '#D946EF', cursor: 'pointer' }}><Pencil /></button></>}
            </div>
