...
export default function TrialActivationScreen({ onAuthenticated, onSkip }) {
  ...
  return (
    <main className="paywall-shell">
      ...
      <style jsx>{`
        .paywall-shell { position: relative; min-height: 100dvh; box-sizing: border-box; background: radial-gradient(circle at 50% 22%, #FBEAFE 0%, #FFF9F4 33%, #FFFDF6 72%); padding: 30px 20px 26px; overflow: auto; animation: screen-in 420ms ease-out both; }
        .paywall-shell::before { content: ''; position: fixed; z-index: 0; inset: 0; pointer-events: none; background: radial-gradient(circle at 50% 23%, rgba(232,121,249,.11), transparent 36%); animation: ambient-settle 850ms ease-out both; }
