...
export function usePaymentProvider() {
  const [state, setState] = useState({ provider: 'paypal', ready: false, products: {} });

  useEffect(() => {
    let disposed = false;
    async function detect() {
      if (typeof window === 'undefined' || typeof window.getDigitalGoodsService !== 'function' || typeof window.PaymentRequest !== 'function') {
        if (!disposed) setState({ provider: 'paypal', ready: true, products: {} });
        return;
      }
      try {
        ...
        if (!disposed) setState({ provider: 'google-play', ready: true, products });
      } catch (error) {
