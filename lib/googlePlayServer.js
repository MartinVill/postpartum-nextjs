...
export async function refreshGooglePlayPurchase({ planType, purchaseToken }) {
  ...
  // ProductPurchaseV2 uses the enum value `PURCHASED` (subscriptions use
  // `SUBSCRIPTION_STATE_*`). Accept the legacy-prefixed value too so an
  // already completed one-time purchase is never rejected by our backend.
  const purchaseState = purchase.purchaseStateContext?.purchaseState || 'UNKNOWN';
  const purchased = ['PURCHASED', 'PURCHASE_STATE_PURCHASED'].includes(purchaseState);
  return { productId, purchaseToken, recurring: false, billingStatus: purchaseState, trialStartedAt: new Date().toISOString(), trialEndsAt: null, nextBillingAt: null, orderId: purchase.orderId || null, accessStatus: purchased ? 'active' : 'inactive' };
}
