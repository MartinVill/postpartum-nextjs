...
export function webhookEntitlementUpdate(eventType, resource) {
  ...
  if (eventType === 'BILLING.SUBSCRIPTION.ACTIVATED') {
    ...
    const trialStartedAt = resource?.start_time || activatedAt.toISOString();
    const trialEndsAt = nextBillingAt || new Date(activatedAt.getTime() + 7 * DAY_MS).toISOString();
    const reminderScheduledFor = new Date(new Date(trialEndsAt).getTime() - 2 * DAY_MS).toISOString();
    return {
