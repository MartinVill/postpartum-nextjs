...
export async function createPayPalLifetimeOrder({ userId, returnUrl, cancelUrl }) {
  const payload = await paypalRequest('/v2/checkout/orders', {
    ...
    body: {
      intent: 'CAPTURE',
      purchase_units: [{
        reference_id: 'postpartum_lifetime',
        custom_id: userId,
        description: 'Postpartum — acceso de por vida',
        amount: { currency_code: 'USD', value: '15.00' }
      }],
      payment_source: { paypal: { experience_context: {
        brand_name: 'Posparto', locale: 'es-AR', user_action: 'PAY_NOW', shipping_preference: 'NO_SHIPPING', return_url: returnUrl, cancel_url: cancelUrl
      } } }
    }
  });
  const approvalUrl = payload.links?.find(link => link.rel === 'payer-action' || link.rel === 'approve')?.href;
  if (!payload.id || !approvalUrl) throw new Error('PayPal did not provide an approval link');
  return { orderId: payload.id, approvalUrl, status: payload.status };
}

export async function capturePayPalLifetimeOrder(orderId, userId) {
  const order = await paypalRequest(`/v2/checkout/orders/${encodeURIComponent(orderId)}/capture`, { method: 'POST', requestId: randomUUID(), body: {} });
  const purchaseUnit = order.purchase_units?.[0];
  const completed = order.status === 'COMPLETED' && purchaseUnit?.custom_id === userId && purchaseUnit?.amount?.currency_code === 'USD' && purchaseUnit?.amount?.value === '15.00';
  if (!completed) throw new Error('PayPal lifetime order is not completed');
  return { orderId: order.id, captureId: purchaseUnit?.payments?.captures?.[0]?.id || null, status: order.status };
}
