...
- Después de la guía se muestra la activación de la prueba de 7 días. La UI ofrece continuar con Google, email o “Ahora no”; el cobro desde esa pantalla aún no está maquetado.
- El fundamento técnico de PayPal Sandbox está validado: planes de USD 5/mes y pago único finito de USD 15, rutas de creación/cancelación autenticadas, webhooks verificados y estructura de acceso en Firestore. Una aprobación real con una cuenta Personal Sandbox dejó el acceso en `trialing`.
- El Paso 6 ya incluye Paywall: timeline dinámico de siete días, selector de pago único/mensual, panel de acceso separado (Google o email), creación segura de la suscripción y redirección a PayPal. El retorno consulta el entitlement confirmado por webhook antes de marcar la prueba como activa en el dispositivo. El correo del día 5 aún requiere un servicio de email y un job programado.
- Ver [PayPal Sandbox y acceso de prueba](./paypal-sandbox-billing.md) para el esquema de colecciones, eventos y validación requerida antes de Live.
